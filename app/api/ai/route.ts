...
export function buildNewsPrompt({
  ...
  return [
    ...
    "CRITICAL: Pronouns (rows[i][0]) MUST be in the TARGET language (e.g. 'yo', 'tú' for Spanish, 'ich', 'du' for German). Do NOT use source language pronouns.",
    `summary: 3-7 sentences forming a single flowing mini-article in ${sourceLabel}. Translate the input text if needed.`,
    `summary_source: The SAME content as 'summary', but in the ORIGINAL language of the input text (${targetLabel}).`,
