...
export function buildNewsPrompt({
  ...
  return [
    ...
    "CRITICAL: Pronouns (rows[i][0]) MUST be in the TARGET language (e.g. 'yo', 'tú' for Spanish, 'ich', 'du' for German). Do NOT use source language pronouns.",
    `summary: 3-7 sentences forming a single flowing mini-article in ${targetLabel} (the target language). Translate the input text into the target language.`,
    `summary_source: The SAME content as 'summary', but in ${sourceLabel} (the source language). This is the original language version.`,
